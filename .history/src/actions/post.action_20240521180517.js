...
export const addPostLike = (data) => {
    return (dispatch) => {
        axios.put(`http://localhost:3000/post${data.id}`, data).then(res => {
...
export const editPost = (data) => {
    return (dispatch) => {
        axios.put(`http://localhost:3000/post/${data[0].id}`, data).then(res => {
            dispatch({ type: EDIT_POST, payload: res.data[0] })
        });
    };
};